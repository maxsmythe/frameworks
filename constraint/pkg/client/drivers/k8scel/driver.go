...
func (d *Driver) AddTemplate(ctx context.Context, ct *templates.ConstraintTemplate) error {
	...
	if ct.Spec.Targets[0].K8sValidation == nil {
		return errors.New("k8sValidation not defined")
	}
	// FRICTION: Note that compilation errors are possible, but we cannot introspect to see whether any
	// occured
	validator := d.compiler.Compile(ct.Spec.Targets[0].K8sValidation)
...
func (d *Driver) Query(ctx context.Context, target string, constraints []*unstructured.Unstructured, review interface{}, opts ...drivers.QueryOpt) ([]*types.Result, *string, error) {
	// The need to mock the version conversion interface stuff here seems brittle, can this be separated out?
	// FRICTION: Resource info is not available shift-left (no API server => no REST mapper and conversion is superfluous for audit, which scrapes all GVKs)
	// FRICTION: the `Attributes` interface defines unnecessary methods. Can we have an interface that defines only required methods to avoid stubbing out functions?
	d.mux.RLock()
	defer d.mux.RUnlock()
	...
	for _, constraint := range constraints {
		...
		decisions, err := validator.Validate(typedReview, &mockObjectInterfaces{}, constraint, typedReview.GetKind())
		if err != nil {
			return nil, nil, err
		}
